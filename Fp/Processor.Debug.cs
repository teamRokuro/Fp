...
namespace Fp
{
    public partial class Processor
    {
        ...
        /// </summary>
        public readonly Dictionary<Memory<byte>, List<(int offset, int length, string label, Color color)>>
            MemAnnotations = new Dictionary<Memory<byte>, List<(int offset, int length, string label, Color color)>>();
        ...
        public void MemLabel(Memory<byte> memory, int offset, int length, string label, Color? color = null)
        {
            if (Debug) return;
        ...
        /// <remarks>No-op if <see cref="Debug"/> is false.<br/>Users should not slice memory struct between label and print, uses <see cref="MemAnnotations"/> which uses the memory as a key.</remarks>
        public void MemPrint(Memory<byte> memory, bool space = true, bool pow2Modulus = true)
        {
            if (Debug) return;
